...
void nextFit(vector<MemoryBlock>& memory, int processId, int processSize, int& lastAllocatedBlock) {
    for (int i = lastAllocatedBlock; i < memory.size(); i++) {
...
int main() {
    ...
    for (int i = 0; i < numProcesses; i++) {
        ...
        cin >> processSize;
        nextFit(memory, i + 1, processSize, lastAllocatedBlock);
    }
